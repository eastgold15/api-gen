# api-gen 架构与 AST 分层扫描规范

> 本文档为设计/架构参考，记录目录分层约定、OXc AST 扫描逻辑与各分层数据提取标准。面向用户的使用说明见根目录 [`readme.md`](../readme.md)。
>
> ⚠️ 历史差异：文中的 `server` 分层在实现中已改名为 `service`，以 `src/utils/tree-builder.ts` 的 `LAYERS` 为准。

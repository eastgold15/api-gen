...
function processSubModule(
  ...
  if (existsSync(indexPath) && !hasAutoGenMarker(indexPath)) {
    pail.warn(`跳过（手动维护）：${chalk.dim(relative(CWD, indexPath))}`);
    return null;
  }
  ...
  if (dryRun) {
    console.log(chalk.dim(`  ~ ${relative(CWD, indexPath)}`));
    ...
    if (!shouldWrite(indexPath, content)) {
      pail.debug(`无变更：${chalk.dim(relative(CWD, indexPath))}`);
    } else {
...
function processGroup(name: string, rootDir: string, dryRun: boolean): void {
  ...
  if (existsSync(groupIndexPath) && !hasAutoGenMarker(groupIndexPath)) {
    pail.warn(`跳过组级 index（手动维护）：${chalk.dim(relative(CWD, groupIndexPath))}`);
    return;
  }

  if (dryRun) {
    console.log(chalk.dim(`  ~ ${relative(CWD, groupIndexPath)}`));
    ...
    if (!shouldWrite(groupIndexPath, groupContent)) {
      pail.debug(`无变更：${chalk.dim(relative(CWD, groupIndexPath))}`);
    } else {
